...
__prep_root__ = prep.root

varoptions = Union[str, list, tuple]

dims_file = __prep_root__ / "static/prms_metadata/dimensions.toml"
control_file = __prep_root__ / "static/prms_metadata/control.toml"
params_file = __prep_root__ / "static/prms_metadata/parameters.toml"
vars_file = __prep_root__ / "static/prms_metadata/variables.toml"
gsflowparams_file = __prep_root__ / "static/prms_metadata/gsflow_parameters.toml"

prms_dtypes = {
    'integer':{'PRMS': 1, 'numpy': np.int32, 'netcdf': 'i4'},
    'real': {'PRMS': 2, 'numpy': np.float32, 'netcdf': 'f4'},
    'double': {'PRMS': 3, 'numpy': np.float64, 'netcdf': 'f8'},
    'string': {'PRMS': 4, 'numpy': str, 'netcdf': 'S1'}
}
...
def load_yaml_file(the_file: pl.Path) -> dict:
    ...
    try:
        ...
        print("yaml package is required for loading metadata from yaml format.")
...
def load_toml_file(the_file: pl.Path) -> dict:
    """
    Load toml file

    Args:
        the_file: metadata toml file

    Returns:
        data: dictionary with metadata in a metadata yaml file
    """

    with pl.Path(the_file).open("rb") as file_stream:
        data = tomli.load(file_stream)
    return data
...
dimensions = _dims_to_tuples(load_toml_file(dims_file))
control = _dims_to_tuples(load_toml_file(control_file))
parameters = _dims_to_tuples(load_toml_file(params_file))
variables = _dims_to_tuples(load_toml_file(vars_file))
gsflow_params = _dims_to_tuples(load_toml_file(gsflowparams_file))
...
def is_available(variable_name: str) -> bool:
    ...
    if (
        ...
        or variable_name in gsflow_params.keys()
    ):
        avail = True
    else:
        avail = False
...
def find_variables(vars: varoptions) -> dict:
    ...
    for variable_name in vars:
        ...
        elif variable_name in gsflow_params.keys():
            variable_dict[variable_name] = gsflow_params[variable_name]
